...
def score_test(df, model, full_pipeline):
    """Function created to calculate the score of the model on the basis of model selected and
    pipeline used.
    ...
    ----------
            ...
            model:
                The model used i.e. final model on which we are calculating score on
            full_pipeline:
                Final pipeline generated which is to be used for processing data
    Returns
    ----------
...
if __name__ == "__main__":
    ...
    logging.info("Starting the run of score.py")
